...
// hangs (e.g. TFLite/EdgeTPU CGO hang), the calling goroutine returns nil
// within the configured per-call timeout instead of blocking forever.
...
// EdgeTPU CGO hang freezes the entire detection pipeline across all cameras.
...
// production this calls os.Exit(1) so docker restarts vedetta with fresh state.
